...
> **Nombre interno de trabajo:** "módulo Koibox" (Koibox/SYHUX = competidor de referencia).
...
Por eso el módulo se **activa por capability flag por tenant** (ADR-K6), no por un `businessType` clavado a belleza, y el vocabulario es neutro: **cliente / profesional / servicio**, nunca estilista/terapeuta.
...
|---|---|---|
| Catálogo de servicios con precio/coste/IVA | ✅ Existe (servicios = `product.kind=SERVICE`, espejo de Holded, `serviceId`) | Se **extiende**, no se crea tabla nueva de servicios (ADR-K1) |
...
|---|---|---|
| Tabla `Service` propia con `priceCents`, `durationMin`… | `product` (kind=SERVICE, espejo Holded) **+ tabla de extensión** `service_scheduling` con `duration_min`, `buffer_before/after`, `staff_required`, `online_bookable`, `family`, flags de canal | El precio/IVA es de Holded (emisor). No duplicar catálogo (ADR-K1) |
...
### ADR-K1 · Modelo de datos: extensión local sobre el catálogo Holded, NO tabla de servicios paralela
...
### ADR-K2 · Cliente/CRM: fuente de verdad local, espejo mínimo a contactos de Holded
...
### ADR-K3 · Fiscalidad de bonos y tarjetas regalo: Holded sigue siendo el emisor
...
### ADR-K4 · Concurrencia de agenda: imposible solapar a nivel de BD (GiST)
...
### ADR-K5 · Reserva online embebible en la web de cada negocio (requisito fundamental)
...
### ADR-K5b · Señal al reservar como palanca anti-no-show (decisión abierta)
...
appointment
  ...
  timeslot (tstzrange)        -- GiST anti-solape (ADR-K4)
...
voucher                      -- bono por sesiones o por importe
  ...
  token (uniq, opaco firmado) -- QR (ADR-K7)
  ...
  sold_ticket_id             -- ticket de la venta (fiscal, ADR-K3)
  created_at
...
- **Venta de bono → Holded (ADR-K3):** al vender el bono se cobra y se registra el documento fiscal en Holded en el momento de la venta. `voucher.sold_ticket_id` guarda el enlace.
- **Canje de bono:** descuenta saldo local, **sin re-emitir** documento por la parte cubierta. Extras al ticket normal.
- **Cliente ↔ contacto Holded:** enlace `holded_contact_id`, solo cuando hace falta factura. El CRM no vive en Holded (ADR-K2).
...
|---|---|---|---|
| **B-koibox-1 · CRM / Ficha de cliente** | Tabla `client` + búsqueda A–Z + alta/edición + historial (compras y citas, ligado a ficha) + saldo de bonos (lectura) + campos RGPD + ficha técnica mínima | — | Firma digital consentimiento; segmentación marketing |
| **B-koibox-2 · Catálogo de servicios extendido** | `service_scheduling` (duración, buffers, `staff_required`, familia, flags de canal Caja/Ticket/Agenda/Online) + panel de edición + recursos | — | Packs/experiencias, cupones (fase 2) |
| **B-koibox-3 · Personal + horarios** | `staff_profile` + `staff_shift` (RRULE) + matriz `staff_skill` | — | Fichaje/control horario, comisiones (fase 2) |
| **B-koibox-4 · Agenda** | Vistas día/semana/mes, columna por profesional, cita=servicio+duración, motor de disponibilidad (retícula 15 min), GiST anti-solape, estados de cita, bloqueos, **cita→caja pre-poblada** | B1,B2,B3 | Lista de espera avanzada, recepción multi-sala (fase 2) |
| **B-koibox-5 · Bonos y tarjetas regalo** | Tipos (sesiones/importe), QR firmado, saldo+caducidad+avisos, venta y canje en caja, venta→Holded (anticipo), crédito de cliente | B1 + caja + **✔ asesor fiscal** | Venta online de bonos (va con B6) |
| **B-koibox-6 · Reserva online embebible** | **Widget insertable en la web de cada negocio** (marca blanca), endpoint público (sin login, CORS por dominio del tenant, anti-abuso), disponibilidad en tiempo real, confirmación email/WhatsApp + alta en agenda, `BookingEngine`, alta de cliente `source=WEB` | B4, B2(flags online), B1(alta pública) | Venta online de bonos; **señal al reservar → decisión abierta ADR-K5b** |
| **B-koibox-7 · Recordatorio de cita** | Aviso automático WhatsApp/SMS/email 24/48h antes | B4, B1 | Post-visita/reseñas, recuperación, campañas (fase 2) |
...
- **No** se implementa lógica fiscal propia para bonos: se confirma con asesor y se registra en Holded (ADR-K3, ADR-008).
- **No** se crea tabla de servicios paralela ni se duplica precio/IVA (ADR-K1).
- **No** se clava el módulo a "peluquería" ni a un `businessType` (ADR-K6). Vocabulario neutro.
...
2. **Lanzar B-koibox-1 (CRM) en Claude Code** con el prompt canónico adjunto (`bloque-koibox-1-crm.md`). Es aditivo, cero riesgo sobre producción, y desbloquea todo lo demás.
