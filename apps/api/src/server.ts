...
async function main() {
  ...
  // En desarrollo arrancamos el worker en el mismo proceso. En
  // producción se separa con `pnpm worker:dev` (otro contenedor del compose).
  let worker: ReturnType<typeof startInitialSyncWorker> | null = null;
  if (env.NODE_ENV !== "production") {
    worker = startInitialSyncWorker();
    app.log.info("initial-sync worker arrancado en el proceso del API");
  }
